...
export const jsonAdapter = {
  ...
  async updateTask(id, patch) {
    ...
    if (draft.band !== undefined) {
      requireOneOf(draft.band, 'band', URGENCY_BANDS, 'today');
      ...
      if (draft.bandSetOn === undefined) draft.bandSetOn = toDayKey(new Date());
    }
    ...
    return updateState((state) =>
      applyPatch(requireById(state.tasks, id, 'task'), draft)
    );
  },
  ...
  async updatePerson(id, patch) {
    return updateState((state) =>
      applyPatch(requireById(state.people, id, 'person'), patch)
    );
  },
  ...
  async updateGoal(id, patch) {
    return updateState((state) => applyPatch(requireById(state.goals, id, 'goal'), patch));
  },
  ...
  async updateMemoryEntry(id, patch) {
    return updateState((state) => applyPatch(requireById(state.memory, id, 'memory entry'), patch));
  },
  ...
  async updateJournalEntry(id, patch) {
    return updateState((state) =>
      applyPatch(requireById(state.journal, id, 'journal entry'), patch)
    );
  },
  ...
  async updateAccount(id, patch) {
    return updateState((state) => applyPatch(requireById(state.accounts, id, 'account'), patch));
  },
  ...
  async createTransaction(input) {
    ...
    const amount = requireMinorUnits(draft.amount, 'amount');
    if (amount < 0) {
      throw new Error('amount must be positive; kind gives the direction');
    }
    const date = requireDayKey(draft.date, 'date');
    const accountId = requireRefOfType(draft.accountId, 'accountId', ['account']);
  ...
  async updateTransaction(id, patch) {
    return updateState((state) =>
      applyPatch(requireById(state.transactions, id, 'transaction'), patch)
    );
  },
  ...
  async updateSyncState(integration, patch) {
    ...
    return updateState((state) => {
      ...
      if (existing !== undefined) {
        Object.assign(existing, patch, { integration: name });
        return existing;
      }
