/**
 * The single answer to "what day is it".
 *
 * Every date in PersonalOS is a day key: the string YYYY-MM-DD, resolved in
 * the user's timezone. Habit ticks, meals, daily logs and snapshots are all
 * filed under one, so if two places in the codebase computed "today"
 * differently, sooner or later they would disagree by a day and nothing would
 * report an error -- the streak would just break on a day you did not miss.
 *
 * That is why this module exists and why nothing else may call
 * `new Date().toISOString().slice(0, 10)`. The server clock is UTC; in Rome
 * its midnight arrives at 1am or 2am, while you are still awake.
 */

const DAY_KEY_SHAPE = /^\d{4}-\d{2}-\d{2}$/;
const MS_PER_DAY = 24 * 60 * 60 * 1000;

// This module must be importable from a client component, so it cannot
// import lib/config/env.js -- that module throws by design when loaded in a
// browser (it also reads secrets). Instead, lib/config/env.js hands the
// configured timezone to setDefaultTimezone() once, on the server, as its
// own side effect. Held on globalThis, not a module-level variable, for the
// same reason as the write queue in ADR-0011: Next.js gives the rsc, ssr and
// route layers their own instance of a server module, so a module-level
// value would be one default per bundle instead of one for the process.
const DEFAULT_TIMEZONE_KEY = Symbol.for('personalos.domain.dates.defaultTimezone');

/** @type {{ [DEFAULT_TIMEZONE_KEY]?: string }} */
const shared = /** @type {any} */ (globalThis);

/**
 * Called once by lib/config/env.js so today() and toDayKey() have a default
 * without this module importing configuration itself.
 *
 * @param {string} timezone
 */
export function setDefaultTimezone(timezone) {
  shared[DEFAULT_TIMEZONE_KEY] = timezone;
}

function resolveDefaultTimezone() {
  const timezone = shared[DEFAULT_TIMEZONE_KEY];
  if (timezone === undefined) {
    throw new Error(
      'today()/toDayKey() was called with no timezone and none is configured yet. ' +
        'On the server this means lib/config/env.js has not been imported on this ' +
        'path; from a client component, pass a timezone explicitly instead.'
    );
  }
  return timezone;
}

/**
 * The day key of an instant, as seen from a timezone.
 *
 * @param {Date} instant
 * @param {string} [timezone] IANA zone; defaults to the configured one.
 * @returns {string} YYYY-MM-DD
 */
export function toDayKey(instant, timezone = resolveDefaultTimezone()) {
  const parts = new Intl.DateTimeFormat('en-CA', {
    timeZone: timezone,
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  }).formatToParts(instant);

  /** @param {string} type */
  const part = (type) => {
    const found = parts.find((candidate) => candidate.type === type);
    if (!found) throw new Error('Intl did not return a ' + type + ' part');
    return found.value;
  };

  return part('year') + '-' + part('month') + '-' + part('day');
}

/**
 * The wall-clock time of an instant, as seen from a timezone. The time-half
 * counterpart to `toDayKey()` -- an instant formatted with the server's own
 * zone instead of the requested one is the same class of bug either way.
 *
 * @param {Date} instant
 * @param {string} [timezone] IANA zone; defaults to the configured one.
 * @returns {string} HH:MM, 24-hour
 */
export function formatTimeInZone(instant, timezone = resolveDefaultTimezone()) {
  const parts = new Intl.DateTimeFormat('en-GB', {
    timeZone: timezone,
    hour: '2-digit',
    minute: '2-digit',
    hourCycle: 'h23',
  }).formatToParts(instant);

  /** @param {string} type */
  const part = (type) => {
    const found = parts.find((candidate) => candidate.type === type);
    if (!found) throw new Error('Intl did not return a ' + type + ' part');
    return found.value;
  };

  return part('hour') + ':' + part('minute');
}

/**
 * Today, in the user's timezone. The function the rest of the app calls.
 *
 * @param {string} [timezone]
 * @returns {string} YYYY-MM-DD
 */
export function today(timezone = resolveDefaultTimezone()) {
  return toDayKey(new Date(), timezone);
}

/**
 * True when the string is a real 24-hour wall-clock time, HH:MM -- "08:30",
 * never "25:70". The one check for a time of day, shared by the store and the
 * classifier so the two can never disagree about what they accept.
 *
 * @param {unknown} value
 * @returns {boolean}
 */
export function isTimeOfDay(value) {
  return typeof value === 'string' && /^([01]\d|2[0-3]):[0-5]\d$/.test(value);
}

/**
 * True when the string is a real calendar day, not merely well shaped.
 * Rejects 2026-02-30 as well as "yesterday".
 *
 * @param {unknown} value
 * @returns {boolean}
 */
export function isDayKey(value) {
  if (typeof value !== 'string' || !DAY_KEY_SHAPE.test(value)) return false;
  return toUtcMidnight(value) !== null;
}

/**
 * Calendar-day arithmetic, done in UTC on purpose: day keys have no time and
 * no zone, so shifting them must not be affected by a daylight saving change.
 *
 * @param {string} dayKey
 * @param {number} days may be negative
 * @returns {string} YYYY-MM-DD
 */
export function shiftDayKey(dayKey, days) {
  const midnight = requireDayKey(dayKey);
  return fromUtcMidnight(new Date(midnight.getTime() + days * MS_PER_DAY));
}

/**
 * The `count` most recent day keys, oldest first, ending on `endDayKey`.
 * Used by every card that looks back over a window.
 *
 * @param {number} count
 * @param {string} endDayKey
 * @returns {string[]}
 */
export function dayKeysEndingAt(count, endDayKey) {
  if (!Number.isInteger(count) || count < 1) {
    throw new Error('count must be a positive integer, received ' + String(count));
  }
  requireDayKey(endDayKey);
  const keys = [];
  for (let offset = count - 1; offset >= 0; offset -= 1) {
    keys.push(shiftDayKey(endDayKey, -offset));
  }
  return keys;
}

/**
 * Every day key from `from` to `to` inclusive, oldest first.
 *
 * @param {string} from
 * @param {string} to
 * @returns {string[]}
 */
export function dayKeyRange(from, to) {
  const start = requireDayKey(from);
  const end = requireDayKey(to);
  if (start.getTime() > end.getTime()) {
    throw new Error('range start ' + from + ' is after end ' + to);
  }
  const keys = [];
  for (let time = start.getTime(); time <= end.getTime(); time += MS_PER_DAY) {
    keys.push(fromUtcMidnight(new Date(time)));
  }
  return keys;
}

/**
 * The Monday-to-Sunday week containing a day key, Monday first.
 *
 * @param {string} dayKey
 * @returns {string[]} the 7 day keys of that week, Monday through Sunday
 */
export function weekDayKeys(dayKey) {
  const midnight = requireDayKey(dayKey);
  // getUTCDay() is 0 = Sunday .. 6 = Saturday; shift so 0 = Monday.
  const daysSinceMonday = (midnight.getUTCDay() + 6) % 7;
  const monday = shiftDayKey(dayKey, -daysSinceMonday);
  return dayKeyRange(monday, shiftDayKey(monday, 6));
}

/**
 * @param {string} dayKey
 * @returns {Date}
 */
function requireDayKey(dayKey) {
  const midnight = toUtcMidnight(dayKey);
  if (midnight === null) {
    throw new Error('not a valid day key: ' + JSON.stringify(dayKey));
  }
  return midnight;
}

/**
 * @param {string} dayKey
 * @returns {Date | null} null when the string is not a real calendar day
 */
function toUtcMidnight(dayKey) {
  if (typeof dayKey !== 'string' || !DAY_KEY_SHAPE.test(dayKey)) return null;
  const [year, month, day] = dayKey.split('-').map(Number);
  const midnight = new Date(Date.UTC(year, month - 1, day));
  return fromUtcMidnight(midnight) === dayKey ? midnight : null;
}

/**
 * @param {Date} midnight
 * @returns {string}
 */
function fromUtcMidnight(midnight) {
  return midnight.toISOString().slice(0, 10);
}
