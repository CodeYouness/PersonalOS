 ...
 * destinations: both file a task, linked to whoever the sentence names.
 *
...
/**
 * Where a task shows on the board: its band, or `overdue` once a `today`
 * task's day has passed. Derived, never stored (lib/domain/derive/tasks.js).
 *
 * @typedef {'overdue'|'today'|'week'|'later'} DisplayBand
 */

/**
 * A thing happening at a specific day and time, from a capture with an
 * explicit date and time or from a synced external calendar.
 *
 * Not a Task: a task has no due date by design (see above). Not an Event: an
 * event is a past-tense log, and an appointment is usually in the future.
 * `origin` is set only when a sync wrote or last touched this appointment --
 * the same ExternalOrigin every synced record carries, and the field a
 * re-sync matches against.
 *
 * @typedef {object} Appointment
 * @property {string} id
 * @property {string} title
 * @property {string} date day key
 * @property {string} startTime HH:MM, 24-hour, no timezone -- same posture as a day key
 * @property {string | null} endTime HH:MM, or null when there is no known end
 * @property {string} calendarLabel free text: the calendar it belongs to, not a closed vocabulary
 * @property {string} note yours; a sync never sets it
 * @property {boolean} confirmed false when a sync no longer sees this appointment in the source but kept it for its note/links (ADR 0014)
 * @property {ExternalOrigin | null} origin set when synced from an external calendar
 * @property {string} createdAt ISO instant
 * @property {string} updatedAt ISO instant
 * @property {'user'|'capture'|'journal'|'integration'|'derived'|'seed'} source
 */
...
/**
 * An ordered, half-open day-key range over which a habit was active. Only
 * the last period in a habit's list may be open (`to: null`) -- archiving
 * closes the current period at today, restoring opens a new one, and the
 * gap in between belongs to no period so it never counts as missed.
 *
 * @typedef {object} HabitPeriod
 * @property {string} from day key, inclusive
 * @property {string | null} to day key, exclusive; null while still active
 */
 ...
 *
 * "Archived" is not a field: it is having no open period. See ADR 0015 and
 * lib/domain/derive/habits.js's isActiveOn().
 *
 ...
 * @property {HabitPeriod[]} periods ordered, non-overlapping, oldest first
 */
...
/**
 * What you ate, filed from a capture into the day it was eaten (ADR 0019).
 * Any number, and the time, may be unknown -- `null`, never zero.
 *
 * @typedef {object} Meal
 * @property {string} id
 * @property {string | null} time HH:MM in the user's timezone; null when unknown
 * @property {string} name
 * @property {number | null} calories its own number, normally the macros at 4/4/9
