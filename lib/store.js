 ...
 * capture. Shared by the capture route and Refile, so a sentence filed
 * either way lands the same. The meal is placed on the day the capture was
 * said, at the moment it was said. With no numbers given it is filed by name
 * with its numbers unknown -- never zero, never invented (ADR 0019).
 *
 * @param {import('./domain/types.js').Capture} capture
 * @param {{ name: string, calories?: number | null, protein?: number | null, carbs?: number | null, fat?: number | null, estimated?: boolean }} meal
 * @returns {Promise<import('./domain/types.js').Meal>}
 */
export async function fileCaptureAsMeal(capture, meal) {
  const said = new Date(capture.createdAt);
  const created = await createMeal({ ...meal, date: toDayKey(said), time: formatTimeInZone(said) });
  await createLink({ from: capture.id, to: created.id, rel: 'about' });
  return created;
}
...
export async function refileCapture(id, destination) {
  ...
  if (TASK_DESTINATIONS.includes(destination)) {
    ...
    const meal = await fileCaptureAsMeal(capture, { name: capture.text });
    recordId = meal.id;
  }
