 ...
 *
 * @param {import('./domain/types.js').Capture} capture
 * @param {{ title?: string, calories?: number | null, protein?: number | null, carbs?: number | null, fat?: number | null, date?: string | null, time?: string | null }} [extracted]
 *   what the classifier extracted for `nutrition`, already checked there
 ...
 * @param {Parameters<typeof fileCaptureAsMeal>[1]} [meal] the model's
 *   estimate when refiling into `nutrition`; without one, by name only
 * @returns {Promise<string | null>} the new produced record's id, or null
 */
export async function refileCapture(id, destination, meal) {
  ...
  if (TASK_DESTINATIONS.includes(destination)) {
    ...
    recordId = (await fileCaptureAsMeal(capture, meal)).id;
  }
