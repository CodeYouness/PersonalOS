...
import { formatTimeInZone, toDayKey } from './domain/dates.js';
import { personNamedIn } from './domain/people.js';
import { refType } from './domain/refs.js';
...
export const createHabit = adapter.createHabit;
export const updateHabit = adapter.updateHabit;
export const reorderHabits = adapter.reorderHabits;
...
export const getAppointments = adapter.getAppointments;
export const getAppointment = adapter.getAppointment;
export const createAppointment = adapter.createAppointment;
export const updateAppointment = adapter.updateAppointment;
export const deleteAppointment = adapter.deleteAppointment;
export const upsertAppointmentByOrigin = adapter.upsertAppointmentByOrigin;
...
export const logHabitValue = adapter.logHabitValue;
export const createMeal = adapter.createMeal;
export const getMeal = adapter.getMeal;
export const updateMeal = adapter.updateMeal;
export const deleteMeal = adapter.deleteMeal;
...
/**
 * Says who a task is owed to: one `involves` link to that person, replacing
 * any before it, or none when `personId` is null. Only the link changes --
 * never the task -- so choosing a person does not lock the capture it came
 * from (ADR 0013). The person is checked before anything is removed, so a
 * refused change leaves the old link in place.
 *
 * @param {string} taskId
 * @param {string | null} personId
 * @returns {Promise<void>}
 */
export async function setTaskPerson(taskId, personId) {
  if (personId !== null && (refType(personId) !== 'person' || (await getPerson(personId)) === null)) {
    throw new Error('no person with id ' + JSON.stringify(personId));
  }
  const current = await getLinks({ from: taskId, rel: 'involves' });
  if (current.length === (personId === null ? 0 : 1) && current.every((link) => link.to === personId)) {
    return;
  }
  // ponytail: several writes above the adapter (ADR 0011) -- two changes
  // racing could leave the task involving two people. The panel lets one
  // person change run at a time; move this into one adapter update if a
  // second writer ever appears.
  for (const link of current) {
    await deleteLink(link.id);
  }
  if (personId !== null) {
    await createLink({ from: taskId, to: personId, rel: 'involves' });
  }
}

/**
 * Completes a task: `completedAt` set, and a `task.completed` event -- the
 * weekly review is made of these. Completing never deletes (docs/domain.md).
 * Refused for a task already completed, so the timeline hears it once.
 *
 * @param {string} id
 * @returns {Promise<import('./domain/types.js').Task>}
 */
export async function completeTask(id) {
  // ponytail: check-then-act above the adapter (ADR 0011) -- two Completes
  // racing could both pass the check and record the event twice. The panel
  // disables its buttons while one is in flight; move the check into the
  // adapter's serialised update if a second writer ever appears.
  const task = await getTask(id);
  if (task === null) throw new Error('no task with id ' + id);
  if (task.completedAt !== null) throw new Error('this task is already completed');

  const completed = await updateTask(id, { completedAt: new Date().toISOString() });
  await recordEvent({ type: 'task.completed', subject: id, source: 'user' });
  return completed;
}

/**
 * Puts a completed task back on the board. A correction, not something that
 * happened to you, so it writes no event -- the timeline keeps the
 * completion it already heard about.
 *
 * @param {string} id
 * @returns {Promise<import('./domain/types.js').Task>}
 */
export async function reopenTask(id) {
  const task = await getTask(id);
  if (task === null) throw new Error('no task with id ' + id);
  if (task.completedAt === null) throw new Error('this task is not completed');
  return updateTask(id, { completedAt: null });
}

/**
 * The capture that produced a record, or null when nothing did (the record
 * was made by hand, or by a sync). A capture points at what it produced
 * with `about`; memory entries use `about` too, so only a capture counts.
 *
 * @param {string} recordId
 * @returns {Promise<import('./domain/types.js').Capture | null>}
 */
export async function getProducingCapture(recordId) {
  const captureId = (await getReferrers(recordId, 'about')).find((ref) => refType(ref) === 'capture');
  return captureId === undefined ? null : getCapture(captureId);
}
...
/**
 * Files a capture as a task: the task and the `about` link back from the
 * capture. Shared by the capture route and Refile, so a sentence filed
 * either way lands the same; both follow it with `linkPersonNamedIn`.
 *
 * @param {import('./domain/types.js').Capture} capture
 * @param {string} title
 * @returns {Promise<import('./domain/types.js').Task>}
 */
export async function fileCaptureAsTask(capture, title) {
  const task = await createTask({ title, source: 'capture' });
  await createLink({ from: capture.id, to: task.id, rel: 'about' });
  return task;
}

/**
 * Files a capture as a meal: the meal and the `about` link back from the
 * capture. Shared by the capture route and Refile, so a sentence filed
 * either way lands the same (ADR 0019):
 *
 * - named by the model's title, else by the sentence;
 * - on the day the model placed it, else the day it was said;
 * - at the time the sentence gave, else the moment it was said when that is
 *   the same day, else unknown -- a meal moved to another day never gets an
 *   invented time;
 * - with whatever numbers the model estimated, `estimated` when there are
 *   any; with none, by name only -- never zero, never invented.
 *
 * @param {import('./domain/types.js').Capture} capture
 * @param {Partial<import('./classify.js').MealFields>} [extracted] what the
 *   classifier extracted for `nutrition`, already checked there
 * @returns {Promise<import('./domain/types.js').Meal>}
 */
export async function fileCaptureAsMeal(capture, extracted = {}) {
  const said = new Date(capture.createdAt);
  const saidOn = toDayKey(said);
  const date = extracted.date ?? saidOn;
  const numbers = {
  ...
  const meal = await createMeal({
    ...numbers,
    name: extracted.title ?? capture.text,
    date,
    time: extracted.time ?? (date === saidOn ? formatTimeInZone(said) : null),
    estimated: Object.values(numbers).some((value) => value !== null),
  });
  await createLink({ from: capture.id, to: meal.id, rel: 'about' });
  return meal;
}

/**
 * Links a record a capture produced to the person its sentence names, if it
 * names exactly one (`personNamedIn`). Never creates a person -- only the
 * user does (ADR 0018). Every capture path that links a person goes through
 * here, so none of them can link the same sentence differently.
 *
 * @param {string} recordId the task or appointment the capture produced
 * @param {string} text the capture's sentence
 * @returns {Promise<void>}
 */
export async function linkPersonNamedIn(recordId, text) {
  const person = personNamedIn(text, await getPeople());
  if (person !== null) {
    await createLink({ from: recordId, to: person.id, rel: 'involves' });
  }
}
 ...
 *
 * @param {import('./domain/types.js').Capture} capture
 * @returns {Promise<import('./domain/types.js').Task | import('./domain/types.js').Goal | import('./domain/types.js').Appointment | import('./domain/types.js').Meal | null>}
 */
export async function getCaptureProducedRecord(capture) {
  ...
  if (TASK_DESTINATIONS.includes(capture.destination)) return getTask(producedId);
  if (capture.destination === 'goals') return getGoal(producedId);
  if (capture.destination === 'appointment') return getAppointment(producedId);
  if (capture.destination === 'nutrition') return getMeal(producedId);
  return null;
}

/**
 * Whether a task, goal, appointment or meal a capture produced has been
 * touched since creation -- by an edit, or by being finished. A task and a
 * goal spell "finished" differently (`completedAt` vs `done`), and an
 * appointment or a meal has no "finished" state at all, so this checks whichever field the record
 * actually has rather than assuming one shape. Undo and Refile (#22, #23)
 * refuse to act once this is true: the user's own work on the record
 * outranks a correction made from the capture log (ADR-0013).
 *
 * @param {import('./domain/types.js').Task | import('./domain/types.js').Goal | import('./domain/types.js').Appointment | import('./domain/types.js').Meal} record
 * @returns {boolean}
 */
export function isCaptureRecordLocked(record) {
  if (record.updatedAt !== record.createdAt) return true;
  if ('completedAt' in record) return record.completedAt !== null;
  if ('done' in record) return record.done === true;
  return false;
}

/**
 * Deletes the record a capture produced, if it has one, along with every
 * link to or from it -- the `about` from the capture, an `involves` to a
 * person. Shared by Delete, Undo and Refile
...
async function retractProducedRecord(capture) {
  ...
  if (TASK_DESTINATIONS.includes(capture.destination)) await deleteTask(record.id);
  else if (capture.destination === 'goals') await deleteGoal(record.id);
  else if (capture.destination === 'appointment') await deleteAppointment(record.id);
  else if (capture.destination === 'nutrition') await deleteMeal(record.id);
}
 ...
 * @param {(capture: import('./domain/types.js').Capture) => Promise<Parameters<typeof fileCaptureAsMeal>[1]>} [estimateMeal]
 *   asked for the model's estimate when refiling into `nutrition`, only once
 *   the refile is known to go ahead; without it, the meal is filed by name
 * @returns {Promise<string | null>} the new produced record's id, or null
 */
export async function refileCapture(id, destination, estimateMeal) {
  ...
  if (TASK_DESTINATIONS.includes(destination)) {
    const task = await fileCaptureAsTask(capture, capture.text);
    recordId = task.id;
    await linkPersonNamedIn(task.id, capture.text);
    ...
    recordId = (await fileCaptureAsMeal(capture, await estimateMeal?.(capture))).id;
  }
