...
import { isDayKey, shiftDayKey, today } from './domain/dates.js';
 ...
 */
export async function classify(text, chosen) {
  if (hasModelAccess()) {
    try {
      const result = await classifyWithModel(text, chosen === undefined ? DESTINATIONS : [chosen]);
      return {
...
function buildFields(destination, extracted, text) {
  ...
  if (destination === 'nutrition') return mealFields(extracted, title);
 ...
 * @returns {MealFields}
 */
function mealFields(extracted, title) {
  ...
  return {
    ...
    carbs: count('carbs'),
    fat: count('fat'),
    date: isDayKey(date) && /** @type {string} */ (date) <= todayKey && /** @type {string} */ (date) >= shiftDayKey(todayKey, -MEAL_DAYS_BACK)
      ? /** @type {string} */ (date)
      : null,
    time: typeof time === 'string' && /^([01]\d|2[0-3]):[0-5]\d$/.test(time) ? time : null,
  };
}
 ...
 */
async function classifyWithModel(text, destinations) {
