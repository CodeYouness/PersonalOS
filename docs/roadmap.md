...
| 9 | **Capture route** | **Done.** `app/api/capture/route.js`. Writes the capture, a memory entry, the links and an event; `task` and `people` file a task linked to the person the sentence names (never creating one, ADR 0018), `goals`, `appointment` and `nutrition` their own record (a meal, estimated by the model when there is one, ADR 0019); the other three file as capture + memory only until their card exists and a real minimal record is possible. |
