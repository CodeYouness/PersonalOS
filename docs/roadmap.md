...
| 9 | **Capture route** | **Done.** `app/api/capture/route.js`. Writes the capture, a memory entry, the links and an event; `task`, `goals` and `appointment` get a destination record, the other four file as capture + memory only until their card exists and a real minimal record is possible. |
